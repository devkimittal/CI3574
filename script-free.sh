...
if [ "$ostype" = "Darwin" ]; then
    ...
    echo "\n"
    echo "Delegate can't run on a windows machine!"
    echo "Paste the below command in either a linux or mac machine"
    echo "Replace the IP of the windows machine"
    command="docker run -d --cpus=1 --memory=2g \
  -e DELEGATE_NAME=docker-delegate \
  -e NEXT_GEN="true" \
  -e DELEGATE_TYPE="DOCKER" \
  -e ACCOUNT_ID=$ACCOUNT_ID \
